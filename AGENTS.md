...
**数据契约 v2：每月一文件（2026-09-08）**：日记从每天一文件改为每月一文件
（`YYYY/YYYY-MM.md`，天内 `## MM-DD 周X` 日段，决策 D13）。所有变换先定位目标日段
再段内进行：同分钟合并作用域收窄到段内（凌晨消息归前一日段，段未必在文件尾）、
日段按日期排序插入（补推乱序不拆段）、封存/撤回按段判断与回收、仅整月撤空才
trash 文件；提醒「今天写没写」从文件存在改为 `writer.hasEntry` 查日段
（`StorageAdapter` 因此新增 `read`，两 adapter 已实现）。附件布局不变。**不做旧
数据迁移**（无存量用户），旧日文件原地保留为历史数据。126 单测全绿。

...
返回 200 + 空文本 = 可用（正弦波无可识别语音，空文本正常）；403 / 错误码 =
免费版受限 → 语音策略按「存原声为默认」落地。
...
| 3   | SDK 在 Obsidian Electron 内兼容性    | ⏳ 插件装好后实测（Phase 1 验证）                                  |
| 4   | 个人版账号建应用                     | ✅ 已验证：个人免费团队可扫码创建                                  |
| 5   | Obsidian 播放 .opus                  | ⏳ Phase 2 存一条语音到 vault 验证                                 |
| 6   | 3 秒时限                             | ✅ 架构已按「handler 轻活、重活异步」设计                          |
| 7   | 扫码创建的应用默认订阅方式           | ✅ 默认即长连接，事件/权限预填均生效，无需后台操作                 |
| 8   | 事件接收 + 表情 + 附件下载全链路     | ✅ 2026-09-01 实测通过（表情需 `reactions:write_only` 权限发布后） |
